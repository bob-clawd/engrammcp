...
public sealed class ConsolidateTool(IMemoryService memoryService) : Tool
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            return mode switch
            {
                "read" => (await memoryService.ReadForMaintenanceAsync(section, cancellationToken).ConfigureAwait(false)).ToConsolidateResponse(),
                "write" => (await ExecuteWriteAsync(section, consolidationToken, entries, cancellationToken).ConfigureAwait(false)).ToConsolidateResponse(),
